...
const REASONING_EFFORT_ORDER = new Map(GPT_5_5_REASONING_EFFORTS.map((effort, index) => [effort, index]))
...
function orderReasoningEfforts(efforts) {
  return uniqueStrings(efforts).sort((left, right) => {
    const leftIndex = REASONING_EFFORT_ORDER.get(left) ?? Number.MAX_SAFE_INTEGER
    const rightIndex = REASONING_EFFORT_ORDER.get(right) ?? Number.MAX_SAFE_INTEGER
...
export function isGpt55ModelId(modelId) {
  const slug = modelIdSlug(modelId)
  return slug === "gpt-5.5" || slug === "gpt-5.5-fast" || slug === "gpt-5.5-codex" || slug === "gpt-5.5-codex-fast"
}
...
function catalogReasoningEfforts(entry, modelId) {
  ...
  const efforts = orderReasoningEfforts(
    rawEfforts
      .map(
        (effort) =>
  ...
  if (isGpt55ModelId(modelId)) return GPT_5_5_REASONING_EFFORTS
  return []
}

function createCatalogModel(modelId, entry, actualModelId = modelId, name = catalogModelName(entry, actualModelId)) {
  const contextLimit =
    isGpt55ModelId(actualModelId) || isGpt55ModelId(modelId)
...
function mergeDiscoveredModels(catalog, account, models) {
  ...
  for (const modelId of accountVisibleModelIds(account, [...discovered.keys()])) {
    ...
    if (isGpt55ModelId(modelId)) registerGpt55Models(catalog, discovered.get(modelId))
  }
}
...
function mergeCachedModels(catalog, account) {
  for (const modelId of accountVisibleModelIds(account, account.availableModels)) {
    ...
    if (isGpt55ModelId(modelId)) registerGpt55Models(catalog)
  }
}
